...
- 2026-05-07: Native NVENC encoder hardened for hybrid AMD+NVIDIA / Blackwell setups (crash1.txt, log12.txt). Java side `NativeOpenGlEncoder.shouldUse` now skips the native path when the active GL renderer is not an NVIDIA part (silent FFmpeg fallback). C++ side `createCudaContext` removes the unsafe `cuDeviceGet(0)` fallback that previously created a CUcontext on a non-OpenGL device, then crashed in `cuGraphicsGLRegisterImage`/`cuCtxPushCurrent`; we now probe `CU_GL_DEVICE_LIST_CURRENT_FRAME / NEXT_FRAME / ALL` and surface a clean exception if no NVIDIA GL device is found. `applyH264Config` drops `outputAUD` (rejected on Blackwell drivers) and switches to VBR with non-zero VBV; `inputBitDepth/outputBitDepth = NV_ENC_BIT_DEPTH_8` are kept under `#ifdef` so older headers still compile. Default header lookup in `build-windows.sh` now prefers v13/v12 over v11/v9 so the rebuilt DLL targets NVENCAPI ≥ 12. DLL rebuilt with NVENC API 12.2 against `nv-codec-headers-12`.
- 2026-05-07: Added `Mixin_LiftReplayTickRateCap` to lift Minecraft's `Math.min(10, ticks)` per-frame cap to 200 while a replay is active. Without this lift, replay speed 8x silently collapsed back toward 1x whenever render-frame time exceeded ~16 ms (TODO.md line 1). Off-replay behaviour is unchanged because the redirect short-circuits to the original `Math.min` when no `ReplayHandler` is active.
- 2026-05-07: `ReplayHandler.doJump` large-jump branch now triggers QuickReplaySender initialization in the background when it starts, so subsequent large jumps can use the O(log n) Quick Mode seek path even if the user never opened Quick Mode manually (TODO.md "イーターの後半30分時点").
- 2026-05-07: `:1.19.4:build` and `:1.21.4:build` passed with the new mixins/encoder fallback. Native DLL rebuilt: `native/replaymod-native-encoder/build/windows/replaymod_native_encoder.dll`.
