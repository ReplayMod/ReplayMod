...
public class GLStateTracker {
    ...
    public EnabledState fog = new EnabledState();
    public EnabledState[] texture = new EnabledState[32];
    {
        for (int i = 0; i < texture.length; i++) {
            texture[i] = new EnabledState();
        }
    }
    ...
    public void updateEnabledState(int magic, boolean enabled) {
        switch (magic) {
            case GL11.GL_FOG: fog.setEnabled(enabled); break;
            case GL11.GL_TEXTURE_2D: texture[activeTexture].setEnabled(enabled); break;
        }
    }
